...
for i in range(5):
    estimates[:,:,i] = np.load('estimates_SVM_'+ str(N[i]) + '.npy')
    CI[:,:,i] = np.load('CI_SVM_'+ str(N[i]) + '.npy')
    SEs[:,:,i] = np.load('SEs_SVM_'+ str(N[i]) + '.npy')
...
labels = ['DR logistic ps, SVM om',
          'DR logistic om, SVM ps',
          'DR SVM',
          'DR wrong logistic om, SVM ps',
          'DR wrong loigstic ps, SVM om',
          'DR wrong SVM om, SVM ps',
          'DR wrong SVM ps, SVM om',
          'DR wrong SVM ps & om',
          'DR exact']
