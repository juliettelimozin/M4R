...
for i in range(iters):
    ...
    W4 = np.random.exponential(1,N)
    A = np.random.binomial(1, expit(-W1 + 2*np.multiply(W1,W2) - W3 + 2*np.multiply(W3,W4)), N)
    Y = np.random.binomial(1, expit(0.2*A-W1 + 2*np.multiply(W1,W2) - W3 + 2*np.multiply(W3,W4)), N)
    W = np.array([W1, W2, W3, W4]).T
...
np.save('estimates_RF_moreW_' + str(N) + '.npy',estimates)
np.save('CI_RF_moreW_' + str(N) + '.npy', CI)
np.save('SEs_RF_moreW_' + str(N) + '.npy', SEs)